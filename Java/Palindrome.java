...
public class Palindrome {
    ...
    public boolean isPalindrome(int x) {
        ...
        while(holder != 0){ //Only single integer will be the evaulated not an array so while loop will suffice
            ...
            reverse = reverse * 10 + lastDigit; //This equation will reverse the integer digit by digit after each loop. If input is 212, first loop will be (reverse = 0 * 10 + 2). Second loop will be (reverse = 2 * 10 + 1), Third will be (reverse = 21 * 10 + 2)
